...
class WTAbsSeason_loc(BaseCardType):
    """
    ...
    TITLE_FONT = str(REF_DIRECTORY / 'TerminalDosis-Bold.ttf')
    TITLE_COLOR = '#FFFFFF'
    ...
    EPISODE_TEXT_FORMAT = "{abs_number:02}"
    ...
    __slots__ = (
        'source_file', 'output_file', 'season_text', 'episode_text', 'font',
        'font_size', 'title_color', 'hide_season', 'vertical_shift', 'interline_spacing',
    ...
    def _add_season_text(self, gradient_image: Path) -> Path:
        """
        Adds episode season title/number to the provide image.

        :param      gradient_image: The image with gradient added.

        :returns:   Path to the created image that has a gradient and the season
                    title/number added.
        """
        ...
        command = ' '.join([
            ...
            *self.__title_text_black_stroke(),
            f'-annotate +0+{vertical_shift} "{self.season_text}"',
    ...
    def create(self) -> None:
        ...
        # Add either one or two lines of season title/number
        titled_image = self._add_season_text(gradient_image)
